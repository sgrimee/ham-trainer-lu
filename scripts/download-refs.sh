...
# Files already present in reference/ are left untouched, so this is safe to re-run.
set -eu
...
yq -r '(.documents // [])[] | [.filename, .url] | @tsv' "$manifest" > "$list" ||
    { echo "could not parse $manifest" >&2; exit 1; }

while IFS="$(printf '\t')" read -r filename url; do
    [ -n "$filename" ] || continue
